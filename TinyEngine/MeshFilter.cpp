...
namespace TEngine {
	...
	Mesh* MeshFilter::processMesh(aiMesh* mesh_, const aiScene* scene) {
		...
		mesh->colors.data = new float[mesh_->mNumVertices * 3]{ 1 };
		mesh->colors.count = mesh_->mNumVertices * 3;
		...
		for (unsigned int i = 0; i < mesh_->mNumVertices; i++) {
			...
			if (mesh_->mColors[0]) {
				mesh->colors.data[i * 3] = mesh_->mColors[i]->r;
				mesh->colors.data[i * 3 + 1] = mesh_->mColors[i]->g;
				mesh->colors.data[i * 3 + 2] = mesh_->mColors[i]->b;
			}
			if (mesh_->mTextureCoords[0]) {
				mesh->uv0.data[i * 3] = mesh_->mTextureCoords[0][i].x;
				mesh->uv0.data[i * 3 + 1] = mesh_->mTextureCoords[0][i].y;
			};
		}
		mesh->triangles.count = 0;
		for (unsigned int i = 0; i < mesh_->mNumFaces; i++) {
			mesh->triangles.count += mesh_->mFaces[i].mNumIndices;
		}
	...
	void MeshFilter::Draw(Camera* camera) {
		if (render == nullptr) {
			...
			for (GLuint i = 0; i < meshes.size(); i++) {
				Material* mat;
				if (render->materials.size() > meshes[i]->materialIndex)
					mat = render->materials[meshes[i]->materialIndex];
				else
					mat = Material::GetDefaultMaterial();
				mat->Use(camera, obj, meshes[i]);
			}
		}
	}
